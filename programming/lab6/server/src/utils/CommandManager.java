...
public class CommandManager {
    ...
    public static void handlePacket(InetSocketAddress sender, byte[] bytes) throws Exception {
        ...
        ServerLogger.getLogger().log(Level.INFO, "Received command %s from %s".formatted(command.getCommandName().toUpperCase(), sender));
        byte[] output = command.run().getResponse().getBytes();
        ServerLogger.getLogger().log(Level.INFO, "Sending output to %s".formatted(sender));
